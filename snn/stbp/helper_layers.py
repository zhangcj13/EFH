...
def replace_bn_by_tdbn(model):
    for name, module in model._modules.items():
        # if isinstance(module,Conv):
        #     model._modules[name] = msBlock(module.conv,module.norm,module.act)
        #     continue
        if hasattr(module,"_modules"):
            model._modules[name] = replace_bn_by_tdbn(module)
        
        if module.__class__.__name__ == 'BatchNorm2d':
            model._modules[name] = tdBatchNorm(num_features=module.num_features,
                                                eps=module.eps, 
