...
from yolox.data import PKUSODDetection
...
class RsHNet(nn.Module):
    ...
    def infer_all_steps(self, data, steps=[0]):
        functional.reset_net(self.event_backbone)
        # fpn output content features of [dark3, dark4, dark5]
        x = data['image']
        evt = data['event']
...
class RsHN_Exp(BaseExp):
    def __init__(self, cfg):
        ...
        self.input_size = cfg.input_size if hasattr(cfg,'input_size') else (256, 352)  # (height, width)
        self.test_size =  cfg.test_size if hasattr(cfg,'test_size') else (256, 352)
        ...
        self.evt_dt= cfg.evt_dt if hasattr(cfg,'evt_dt') else 4900
        self.eval_us = cfg.eval_us if hasattr(cfg,'eval_us') else 34300
        self.enable_mosaic = cfg.enable_mosaic if hasattr(cfg,'enable_mosaic') else False

        self.cfg = cfg

        self.output_dir = "./YOLOX_outputs/RsHN_PKUSOD/"+ self.cfg.net_type + '/' + cfg.arch_net
    ...
    def get_eval_dataset(self, **kwargs):        
        ...
        eval_set = PKUSODDetection(
                            data_root=self.cfg.data_root, 
                            split=split, 
                            transform =  ValTransform_FaE(legacy=legacy),
                            min_bbox_height=1, # self.cfg.min_bbox_height,
                            min_bbox_diag=1, #self.cfg.min_bbox_diag,
                            num_us=self.evt_dt*self.cfg.total_time_steps,
                            dt=self.evt_dt, #us
