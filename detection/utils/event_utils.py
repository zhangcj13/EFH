...
def view_event_boxes_on_image(image,event,boxes,save_path='/root/data1/ws/SNN_CV/YOLOX_outputs/gt_imgs/_temp.jpg',last_ts=0):
    DSEC_CLASSES = ("car", "pedestrian",'two_wh')
    if isinstance(image,torch.Tensor):
        image = image.cpu().numpy()[0]
    if isinstance(event,torch.Tensor):
        event = event.cpu().numpy()[0]
    if isinstance(boxes,torch.Tensor):
        boxes = boxes.cpu().numpy()[0]    
    
    draw_img = np.ascontiguousarray(image.transpose((1,2,0)) if image.shape[0]==3 else image, dtype=np.float32)
    ih,iw,c = draw_img.shape
    mask_p = np.zeros((ih,iw),dtype=np.uint8)
    mask_n = np.zeros((ih,iw),dtype=np.uint8)
    
    if len(event)>0:
        _,_,_,ts = event.shape
        start_ts = 0
        if last_ts>0:
            start_ts = max(ts-last_ts,0)
        for t in range(start_ts,ts,1):
            mask_p += (event[0,:,:,t]>0).astype(np.uint8)
            mask_n += (event[1,:,:,t]>0).astype(np.uint8)
        
        draw_img[mask_p>0,:]=(255,0,0)
        draw_img[mask_n>0,:]=(0,0,255)
    ...
    for box in boxes:
        ...
        cv2.rectangle(draw_img,(int(l), int(t)), (int(r), int(b)), (0, 255, 0), 1)
        cls = DSEC_CLASSES[int(c)]
        cv2.putText(draw_img, cls, (int(l), int(t+12)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1, cv2.LINE_AA)
    
    if save_path is not None:
        cv2.imwrite(save_path,draw_img.astype(np.uint8))
    # print(img_info)
