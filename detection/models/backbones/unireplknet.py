...
from detection.utils.net_utils import load_state_dict
...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias,
               attempt_use_lk_impl=False #True
               ):
...
def load_with_key(model, key):
    ...
    cache_file = os.path.join('./pretrained/unireplknet',huggingface_file_names[key])
    checkpoint = torch.load(cache_file, map_location='cpu')

    if 'model' in checkpoint:
        checkpoint = checkpoint['model']
    # model.load_state_dict(checkpoint)
    load_state_dict(model, checkpoint, prefix='')
...
class unireplknet_a(UniRepLKNet):
    def __init__(self, **kwargs):
        super(unireplknet_a, self).__init__(depths=UniRepLKNet_A_F_P_depths, dims=(40, 80, 160, 320), **kwargs)

        initialize_with_pretrained(self, 'unireplknet_a', True, False, False)

@BACKBONES.register_module
class unireplknet_f(UniRepLKNet):
    def __init__(self, **kwargs):
        super(unireplknet_f, self).__init__(depths=UniRepLKNet_A_F_P_depths, dims=(48, 96, 192, 384), **kwargs)

        initialize_with_pretrained(self, 'unireplknet_f', True, False, False)

@BACKBONES.register_module
class unireplknet_p(UniRepLKNet):
    def __init__(self, **kwargs):
        super(unireplknet_p, self).__init__(depths=UniRepLKNet_A_F_P_depths, dims=(64, 128, 256, 512), **kwargs)

        initialize_with_pretrained(self, 'unireplknet_p', True, False, False)

@BACKBONES.register_module
class unireplknet_n(UniRepLKNet):
    def __init__(self, **kwargs):
        super(unireplknet_n, self).__init__(depths=UniRepLKNet_N_depths, dims=(80, 160, 320, 640), **kwargs)

        initialize_with_pretrained(self, 'unireplknet_n', True, False, False)

@BACKBONES.register_module
class unireplknet_t(UniRepLKNet):
    def __init__(self, **kwargs):
        super(unireplknet_t, self).__init__(depths=UniRepLKNet_T_depths, dims=(80, 160, 320, 640), **kwargs)

        initialize_with_pretrained(self, 'unireplknet_t', True, False, False)

@BACKBONES.register_module
class unireplknet_s(UniRepLKNet):
    def __init__(self, **kwargs):
        super(unireplknet_s, self).__init__(depths=UniRepLKNet_S_B_L_XL_depths, dims=(96, 192, 384, 768), **kwargs)

        initialize_with_pretrained(self, 'unireplknet_s', True, False, False)
